...
__device__ void final(SHA256_CTX *ctx, uint8_t *out)
{
	uint32_t i;
	...
	for (i = 0; i < 4; ++i) {
		...
		out[i + 20] = (ctx->state[5] >> (24 - i * 8)) & 0x000000ff;
		out[i + 24] = (ctx->state[6] >> (24 - i * 8)) & 0x000000ff;
		out[i + 28] = (ctx->state[7] >> (24 - i * 8)) & 0x000000ff;
	}
}
